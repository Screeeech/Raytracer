...
Matrix::Matrix(const Vector4 &xAxis, const Vector4 &yAxis, const Vector4 &zAxis,
               const Vector4 &t) {
  data[0] = xAxis;
  data[1] = yAxis;
  data[2] = zAxis;
...
Vector4 Matrix::TransformPoint(float x, float y, float z, float w) const {
  return Vector4{data[0].x * x + data[1].x * y + data[2].x * z + data[3].x,
                 data[0].y * x + data[1].y * y + data[2].y * z + data[3].y,
                 data[0].z * x + data[1].z * y + data[2].z * z + data[3].z,
...
const Matrix &Matrix::Inverse() {
  ...
  for (uint8_t col = 0; col < 4; col++) {
    // Find the row with the largest absolute value in current column (partial
    // pivoting).
    int pivot_row = col;
...
Matrix Matrix::CreatePerspectiveFovLH(float fov, float aspect, float zn,
                                      float zf) {
  // TODO
  return {};
}
...
Matrix Matrix::CreateRotationY(float yaw) {
  return {{static_cast<float>(cos(yaw)), 0, static_cast<float>(-sin(yaw)), 0},
          {0, 1, 0, 0},
...
Matrix Matrix::CreateRotationZ(float roll) {
  return {{static_cast<float>(cos(roll)), static_cast<float>(sin(roll)), 0, 0},
